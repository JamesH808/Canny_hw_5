...
/* CENG 381 Assignment 3 solution */
...
SC_MODULE(Stim){
	...
	void action(){
		...
		for(int i= 1; i<=30; i++){
	  	         ...
	  	         sprintf(infilename, "video/Keck%03d.pgm",i);
	  		 /****************************************************************************
	...
	void action(){
		...
		for(int i= 1; i<=30; i++){
			...
			/****************************************************************************
   			...
   			****************************************************************************/
			 sprintf(outfilename, "Keck%03d_edges.pgm", i);
...
SC_MODULE(Platform){
	...
	sc_fifo_in<IMAGE> i_in;
	sc_fifo_out<IMAGE> i_out;
	sc_fifo<IMAGE> ch1;
        sc_fifo<IMAGE> ch2;

	SC_MODULE(DataIn){
		sc_fifo_in<IMAGE> pd_in;
                sc_fifo_out<IMAGE> pd_out;
	...
	SC_MODULE(DataOut){
                sc_fifo_in<IMAGE> pdo_in;
                sc_fifo_out<IMAGE> pdo_out;
	...
	SC_MODULE(Dut){
		...
		*******************************************************************************/
		void apply_hysteresis(short int *mag, unsigned char *nms, int rows, int cols,
			float tlow, float thigh, unsigned char *edge)
		{
   			int r, c, pos, numedges, lowthreshold, highthreshold, highcount, hist[32768];
  			/* int lowthreshold = 50;
	...
	{
		datain1.pd_in.bind(i_in);
		datain1.pd_out.bind(ch1);
		dut1.p_in.bind(ch1);
		dut1.p_out.bind(ch2);
		dataout1.pdo_in(ch2);
		dataout1.pdo_out(i_out);
	}
...
SC_MODULE(Top){
	...
	/*TODO: PUT PORT BINDING HERE*/
	stim1.p_out.bind(ch1);
	plat1.i_in.bind(ch1);
	plat1.i_out.bind(ch2);
	mon1.p_in.bind(ch2);
